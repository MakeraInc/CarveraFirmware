...
void CartGridStrategy::print_bed_level(StreamOutput *stream)
{
    ...
    if(!human_readable){
        for (int y = 0; y < current_grid_y_size; y++) {
            for (int x = 0; x < current_grid_x_size; x++) {
                buffer_pos += snprintf(buffer + buffer_pos, sizeof(buffer) - buffer_pos, "%1.4f ", grid[x + (current_grid_x_size * y)]);                             
        ...
        for (int y = yStart; y != yStop; y += yInc) {
            buffer_pos += snprintf(buffer + buffer_pos, sizeof(buffer) - buffer_pos, "%10.4f|", y * (y_size / (current_grid_y_size - 1))); 
            for (int x = xStart; x != xStop; x += xInc) {
                buffer_pos += snprintf(buffer + buffer_pos, sizeof(buffer) - buffer_pos, "%10.4f ",  grid[x + (current_grid_x_size * y)]);                              
		        if (buffer_pos >= sizeof(buffer) - 30) {
		            stream->printf("%s", buffer);
		            buffer_pos = 0;
		        } 
        ...
        buffer_pos += snprintf(buffer + buffer_pos, sizeof(buffer) - buffer_pos, "           ");
        for (int x = xStart; x != xStop; x += xInc) {
            buffer_pos += snprintf(buffer + buffer_pos, sizeof(buffer) - buffer_pos, "%1.4f ",  x * (x_size / (current_grid_x_size - 1)));                              
		        if (buffer_pos >= sizeof(buffer) - 30) {
		            stream->printf("%s", buffer);
		            buffer_pos = 0;
		        } 
